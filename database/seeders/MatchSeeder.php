...
class MatchSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Insert male matches
        foreach ($scheduleMale as $match) {
            LeagueMatch::create([
                'jornada' => $match['jornada'],
                'date' => $this->parseDate($match['fecha']),
                'time' => $match['hora'] ?: null,
                'local_team' => $match['local'],
                'visitor_team' => $match['visitante'],
                'court' => $match['cancha'] ?: null,
                'result' => $match['resultado'] ?: null,
                'category' => 'male',
            ]);
        }

        // Insert female matches
        foreach ($scheduleFemale as $match) {
            LeagueMatch::create([
                'jornada' => $match['jornada'],
                'date' => $this->parseDate($match['fecha']),
                'time' => $match['hora'] ?: null,
        ...
        $this->command->info('Seeded ' . count($scheduleMale) . ' male matches and ' . count($scheduleFemale) . ' female matches.');
    }
